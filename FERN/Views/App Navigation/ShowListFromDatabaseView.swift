...
struct ShowListFromDatabaseView: View {
            ...
            NavigationStack {
                List (self.list) { (item) in
                    NavigationLink(item.name) {
                        ...
                        SelectMapUILayoutView(map: map, gps: gps, camera: camera, mapMode: mapMode, tripOrRouteName: item.name, columnName: columnName, organismName: organismName, queryName: queryName)
    ...
    private func getListOfTripsInDatabase() async {
        ...
        self.list = await menuListBridgingCoordinator.menuListController.getTripListFromDatabase(settings: settings, nameList: list, phpFile: "menusAndReports.php", isMethodPost: true, postString: "_query_name=trips_in_db_view")
    }
