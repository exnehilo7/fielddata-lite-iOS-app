...
@Observable class RoutingCache {
    ...
    var totalRouteFiles = 0
    var processedRouteFiles = 0
    var menuListClass = MenuListClass()
    
    func refreshCache(settings: [Settings], map: MapClass) async -> Bool {
        
        showProgressBar = true
        
        guard let dir = DocumentsDirectory.dir else {return false}
        var filePath: URL
        var list: [SelectNameClass] = []
...
//        upload.appendToTextEditor(text: "🔃 Refreshing cache for routing maps...")
        ...
        list = await menuListClass.getTripListFromDatabase(settings: settings, nameList: list, phpFile: "menuLoadSavedRouteView.php", isMethodPost: false)
        
        totalRouteFiles = list.count
        ...
        for menuItem in list {
            ...
            processedRouteFiles += await writeMapDataToJSONFile(settings: settings, map: map, tripOrRouteName: menuItem.name, columnName: "", organismName: "", queryName: "query_get_route_for_app")
        }
...
//        tea.appendToTextEditor(text: "🟢 Offline cache is refreshed!")
    ...
    // For map data
    private func writeMapDataToJSONFile(settings: [Settings], map: MapClass, tripOrRouteName: String, columnName: String, organismName: String, queryName: String) async -> Int {
